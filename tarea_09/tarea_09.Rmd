...
```{r}
create_log_Gauss_Distribution <- function(mu, sig){
  ...
  function(xi){
    ...
    fx <- 1/(v)*exp(-0.5*(u)^2)
    # usando log
    log_fx <- (0 - log(sig*sqrt(2*pi)) - 0.5*u^2)
  }
...
La desviacion estandard debe estar como aprox 1.3 (8/6) y el promedio aprox. 5.
