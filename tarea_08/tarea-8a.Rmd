...
sample_poisson_n50_tbl <- tibble(sample_poisson_n50 = rpois(50, 3))
pois_plot <- ggplot(sample_poisson_n50_tbl, aes(x=sample_poisson_n50)) +
  geom_histogram(bins = 15)
sample_poisson_n50_tbl_sum <- sample_poisson_n50_tbl %>% 
  mutate(theta = exp(-2*sample_poisson_n50)) %>% 
...
calc_confidence_interval <- function(data){
  results <- data %>% 
    mutate(theta = exp(-2*data$sample_poisson_n50)) %>% 
    summarise(theta_mean = mean(theta), n = length(theta)) %>% 
    pull(theta_mean)
...
n_boot <- 1000
# n_boot runs
boot_poisson_sample <- map_dbl(1:n_boot, ~calc_confidence_interval(sample_boot(sample_poisson_n50_tbl)))
# calculate values
# make it a df
...
mean_orig_sample = sample_poisson_n50_tbl_sum$theta_mean
# true value
P_true = exp(-2*3)
...
set.seed(38972938)
## simuation
simutator_intervals <- function(rep){
  sample_pois <- tibble(sample_pois = rpois(50, 3))
  mean_theta_estim <- mean(exp(-2*sample_pois$sample_pois)) 
  ...
  calc_estimator <- function(data){
   results <- data %>% 
     mutate(theta = exp(-2*sample_pois)) %>% 
     summarise(theta_mean = mean(theta), n = length(theta)) %>% 
     pull(theta_mean)
   results
  }
...
for(idx in 1:500){
  ...
  # normal
  if((eval_me$low < mean_orig_sample) && (eval_me$up > mean_orig_sample)){
    counter_inside = counter_inside+1
  }
  else if(eval_me$low > mean_orig_sample){
      counter_outside_left = counter_outside_left+1
  }
  else if(eval_me$up < mean_orig_sample){
      counter_outside_right = counter_outside_right+1
    }
  # quantiles
  if((eval_me$theta_q025 < mean_orig_sample) && (eval_me$theta_q975 > mean_orig_sample)){
    counter_inside_qunantile = counter_inside_qunantile+1
  }
  else if(eval_me$theta_q025 > mean_orig_sample){
      counter_outside_left_qunantile = counter_outside_left_qunantile+1
  }
  else if(eval_me$theta_q975 < mean_orig_sample){
