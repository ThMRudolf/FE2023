...
sample_poisson_n50_tbl_sum <- sample_poisson_n50_tbl %>% 
  summarise(theta_mean = exp(-2*mean(sample_poisson_n50)), n = length(data))
pois_plot
```
...
calc_confidence_interval <- function(data){
  results <- data %>% 
    summarise(theta_mean = exp(-2*mean(data$sample_poisson_n50)), n = length(data)) %>% 
    pull(theta_mean)
...
boot_interval_norm <- tibble('2.5%' = interval_boot_poisson_sample$inf, 
       '97.5%' = interval_boot_poisson_sample$sup)
...
simutator_intervals <- function(rep){
  ...
  calc_estimator <- function(data){
   results <- data %>% 
     summarise(theta_mean = exp(-2*mean(sample_pois)), n = length(data)) %>% 
     pull(theta_mean)
   results
  }
