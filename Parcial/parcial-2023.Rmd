...
Ha: El materialB no es comparable con el material A

```{r}
diff_material <- zapatos %>% 
  #mutate(material_type = as.character(material)) %>%
  #group_by(material_type) %>% 
  pivot_wider(names_from = material, values_from = desgaste) %>% 
  #summarise(mean_wear = mean(zapatos$desgaste))
  unlist( recursive = FALSE)
  
diff_material_obs <- mean(diff_material$'2') -  mean(diff_material$'1')
diff_material_obs

# permutación
reps <- lineup(null_permute("material"), zapatos, n = 50000)
#decrypt("nsW7 Ykjk l3 gCPljlC3 4TmKm")
glimpse(reps)
