...
get_boot <- function(sample2boot, strata_id){
  ...
  bb_samples <- slice_sample(muestra_tidy[bb_next, ], n = 1745, replace = TRUE) 
  
  #p_boot <- perc_calc(bb_samples, computos_tidy,bb_next )
  #results_boot <- c(p_boot$p_hat_si,  p_boot$p_hat_no,  p_boot$np_hat_nu, p_boot$check)
}
n_strata = length(strata_array)
...
results_boot<- map(1:2, ~ perc_calc(bootstrat_sample[[.x]], computos_tidy, strata_array))
```
